...
def handle_debit_query(customer_id, transaction_period):
    """
    Retrieves debit transactions for a customer in a given day, month, or year.
    ...
    if not rows:
        return f"❌ No debit transactions found for Customer ID {customer_id} in {transaction_period}."

    response = f"\n📄 Debit Transactions for {customer_id} in {transaction_period}:\n"
    for row in rows:
        try:
            response += (
                f"🗓️ Date: {row[0]}\n"
